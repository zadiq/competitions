...
import json
import warnings
from credit.utils import get_file, timer, dump_json
...
class CurateData:
    ...
    def gen_data(self, debug=False, version='v0'):
        ...
        with timer("Saving data"):
            ...
            df[df['TARGET'].notnull()].to_csv(get_file('org_train', version), index=False)
            df[df['TARGET'].isnull()].to_csv(get_file('org_test', version), index=False)

        return df

    @staticmethod
    def apply_operator(a, df):
        return df.apply(lambda x: ([a + x] + [a - x] + [a / x] + [a * x]))
    ...
    def gen_genetic_data(self, df=None, version='v0'):
        ...
        operators = ['+', '-', '/', '*']
        df = df if df is not None else pd.read_csv(get_file('all_data', version))
        original_genes = df.drop(['SK_ID_CURR', 'TARGET'], axis=1).columns.tolist()
        genes_map = {}
        ...
        for ix, og in enumerate(original_genes[:-1]):
            ...
            genes_map.update(dict(zip(gen_cols, [ix for _ in range(len(gen_cols))])))
            gene_bin['SK_ID_CURR'] = df['SK_ID_CURR']
            gene_bin.to_csv(os.path.join(bin_dir, bin_file.format(ix=ix)), index=False)
...
class BoardProbabilities:
    """
    Generate prediction table from models in board
    """

    def __init__(self, version='v0'):
        self.train_data = pd.read_csv(get_file('org_train', v=version), index_col='index')
        self.test_data = pd.read_csv(get_file('org_test', v=version), index_col='index')
        ...
        self.models = {'Model_{}'.format(ix): lgb.Booster(model_file=m)  for ix, m in  enumerate(self.board)}
        ...
        if self.train_prob.isin([np.inf, -np.inf]).any().any() or self.train_prob.isna().any().any():
            raise ValueError('Problem in data integrity!')

        self.train_prob.to_csv(get_file('board_train_prob', version), index=False)
        self.test_prob.to_csv(get_file('board_test_prob', version), index=False)
...
if __name__ == '__main__':
    ...
    sample = pd.DataFrame(
        {
            ...
            'TARGET': [np.random.randint(0, 2) for _ in range(sample_size)],
    ...
    _ = sample
    data.gen_genetic_data(sample, version='v1')
